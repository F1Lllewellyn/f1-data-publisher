...
def run_pipeline(event_id: str, race_name: str, gate: str, run_source_closure: bool, season: int, dry_run: bool) -> List[Dict[str, Any]]:
    ...
    if run_source_closure:
        if (ROOT / "scripts" / "openf1" / "publish_openf1_lightweight_source_closure.py").exists():
            ...
            # Fail fast if the required source refresh fails. Do not create or lock
            # forecast bundles from stale source files during a real automated run.
            if results[-1].get("returncode") != 0:
                return results
        else:
            results.append({"label": "OpenF1 Lightweight Source Closure", "returncode": 1, "skipped": True, "reason": "required_source_closure_script_missing"})
            return results
...
def main() -> int:
    ...
    try:
        ...
        if failed and failed[0].get("label") == "OpenF1 Lightweight Source Closure":
            result["reason"] = "source_refresh_failed_no_bundles_locked"
        else:
