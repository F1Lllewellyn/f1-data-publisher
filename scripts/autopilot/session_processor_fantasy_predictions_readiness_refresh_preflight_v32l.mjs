...
const DEFAULT_POLICY = 'scripts/autopilot/session_processor_fantasy_predictions_readiness_refresh_preflight_policy_v32l.json';
const DEFAULT_OUT = 'health/session_processor_fantasy_predictions_readiness_refresh_preflight_v32l_status.json';
...
function main() {
  ...
  const racePreflight = readJsonMaybe(args.racePreflight);
  const blockers = [];
  const raceStatus = statusOf(racePreflight);
  const raceQuality = String(racePreflight?.readiness_quality || '');

  if (!racePreflight) blockers.push('missing_v32k_race_preflight');
  else if (raceStatus !== policy.required_race_preflight_status) blockers.push(`race_preflight_status_mismatch:${raceStatus}`);
  else if (raceQuality !== policy.required_race_preflight_quality) blockers.push(`race_preflight_quality_mismatch:${raceQuality || 'missing'}`);

  blockers.push(...governanceIssues('race_preflight', racePreflight));

  if (args.execute) blockers.push('execute_true_not_supported_in_v32l_preflight');
  if (args.activate) blockers.push('activate_true_not_supported_in_v32l_preflight');
  if (args.allowLive) blockers.push('allow_live_true_not_supported_in_v32l_preflight');
  ...
  const output = {
    schema_version: 'f1_session_processor_fantasy_predictions_readiness_refresh_preflight_v32l_2026-06-17',
    ...
    fantasy_predictions_readiness_refresh_preflight_status: ok ? 'fantasy_predictions_readiness_refresh_preflight_ready' : 'fantasy_predictions_readiness_refresh_preflight_blocked',
    readiness_quality: ok ? 'race_predictions_preflight_verified_no_fantasy_refresh_performed' : 'blocked_or_missing_race_predictions_preflight',
    race_preflight_status: raceStatus,
    race_preflight_quality: raceQuality || null,
    blockers,
    consumer_gates: {
    ...
    next_step: ok ? (policy.next_layer || 'v32m_material_change_notification_preflight_after_fantasy_preflight') : 'resolve_v32l_fantasy_preflight_blockers_before_v32m'
  };
  ...
  console.log(JSON.stringify({
    ok,
    out: args.out,
    fantasy_predictions_readiness_refresh_preflight_status: output.fantasy_predictions_readiness_refresh_preflight_status,
