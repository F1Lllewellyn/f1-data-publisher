...
const DEFAULT_POLICY = 'scripts/autopilot/session_live_source_hardening_policy_v30q.json';
const DEFAULT_OUT = 'health/session_live_source_failure_taxonomy_v30q_status.json';
const DEFAULT_LOG = '';
...
function parseArgs(argv) {
  ...
  for (let i = 2; i < argv.length; i += 1) {
    ...
    else if (key === '--validation' && value) { args.validation = value; i += 1; }
    else if (key === '--probe' && value) { args.probe = value; i += 1; }
    else if (key === '--cache' && value) { args.cache = value; i += 1; }
...
function classifyEvidence(source, policy) {
  const failures = [];
  const warnings = [];
  let rowEvidence = 0;
  ...
  for (const item of source.evidence) {
    ...
    if (status.includes('429') || error.includes('429') || obj.http_status === 429) failures.push('rate_limited_http_429');
    if (obj.http_status >= 500 || status.includes('5xx') || error.includes('5xx')) failures.push('provider_http_5xx');
    if (obj.http_status >= 400 && obj.http_status < 500 && obj.http_status !== 429) failures.push('provider_http_4xx');
    if (status.includes('schema') || error.includes('schema')) failures.push('schema_validation_failure');
...
function deriveOverall(classifications, governanceIssues) {
  ...
  if (classifications.length === 0) return { status: 'contract_ready', quality: 'no_live_results_to_classify' };
  return { status: 'hardening_ready', quality: 'no_blocking_source_failures_detected' };
}

function governanceIssues(policy, args) {
  const issues = [];
  ...
  if (args.allowLive && args.mode !== 'sandbox_live') issues.push('allow_live_requires_sandbox_live_mode');
  if (args.allowLive && policy.live_fetch_allowed_by_default !== false) issues.push('live_fetch_default_not_false');
  return issues;
}
...
function main() {
  ...
  const adapter = readJsonMaybe(args.adapter);
  const validation = readJsonMaybe(args.validation);
  const probe = readJsonMaybe(args.probe);
  const cache = readJsonMaybe(args.cache);
  const sources = collectSources(adapter, validation, probe, cache);
  const issues = governanceIssues(policy, args);
  const classifications = sources.map((source) => classifyEvidence(source, policy));
  ...
  const status = {
    ...
    taxonomy_status: overall.status,
    readiness_quality: overall.quality,
    governance_issues: issues,
    source_count: classifications.length,
    blocking_source_count: classifications.filter((c) => c.severity === 'blocking').length,
    degraded_source_count: classifications.filter((c) => c.severity === 'degraded_with_fallback').length,
    classifications,
    consumer_gates: {
  ...
  console.log(JSON.stringify({
    ...
    blocking_source_count: status.blocking_source_count,
    degraded_source_count: status.degraded_source_count
  }, null, 2));
}
