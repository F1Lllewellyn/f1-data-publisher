#!/usr/bin/env python3
"""
Download latest successful OpenF1 autopilot artifacts for Elite Weekend Engine v2.

Fix: GitHub artifact archive_download_url returns a redirect to a temporary
signed URL. Authenticate to the GitHub API endpoint, capture the redirect, then
download the signed URL without the GitHub Authorization header. Carrying the
GitHub token into the signed artifact URL can trigger a 401 from the storage
backend.
"""

import argparse
import json
import os
import sys
import zipfile
from pathlib import Path
from datetime import datetime, timezone
from urllib.request import Request, urlopen
from urllib.error import HTTPError, URLError


PROFILES = {
    "pre_race": [
        "openf1-prerace-auto-final",
        "openf1-high-frequency-2026-prerace",
    ],
    "full_historical": [
        "openf1-full-historical-auto-final",
        "openf1-high-frequency-2026-all",
    ],
    "post_race": [
        "openf1-post-race-auto-final",
    ],
}


class NoRedirectHandler:
    # Small adapter object for urllib opener to stop automatic 302 following.
    pass


def gh_headers(token: str) -> dict:
    return {
        "Authorization": f"Bearer {token}",
        "Accept": "application/vnd.github+json",
        "X-GitHub-Api-Version": "2022-11-28",
        "User-Agent": "f1-elite-weekend-engine-v2",
    }


def gh_get_json(url: str, token: str):
    req = Request(url, headers=gh_headers(token))
    with urlopen(req, timeout=60) as r:
        return json.loads(r.read().decode("utf-8"))


def get_artifact_redirect_url(url: str, token: str) -> str:
    """
    Request the GitHub artifact archive endpoint and return the signed redirect URL.

    urllib follows redirects by default, which can accidentally attach headers to
    the redirected storage URL. We intentionally catch the 302 and return the
    Location header.
    """
    import urllib.request

    class NoRedirect(urllib.request.HTTPRedirectHandler):
        def redirect_request(self, req, fp, code, msg, headers, newurl):
            return None

    opener = urllib.request.build_opener(NoRedirect)
    req = Request(url, headers=gh_headers(token))

    try:
        with opener.open(req, timeout=60) as r:
            # Rare case: GitHub may return the zip directly.
            return url
    except HTTPError as e:
        if e.code in (301, 302, 303, 307, 308):
            location = e.headers.get("Location")
            if not location:
                raise RuntimeError("GitHub artifact redirect response did not include a Location header.")
            return location
        raise


def download_signed_url(url: str, dest: Path):
    """
    Download the signed artifact URL without GitHub Authorization headers.
    """
    req = Request(url, headers={"User-Agent": "f1-elite-weekend-engine-v2"})
    with urlopen(req, timeout=600) as r:
        dest.write_bytes(r.read())


def gh_download_artifact(archive_download_url: str, token: str, dest: Path):
    signed_url = get_artifact_redirect_url(archive_download_url, token)
    if signed_url == archive_download_url:
        # Direct download from GitHub API endpoint; auth is required.
        req = Request(archive_download_url, headers=gh_headers(token))
        with urlopen(req, timeout=600) as r:
            dest.write_bytes(r.read())
    else:
        download_signed_url(signed_url, dest)


def list_artifacts(repo: str, token: str):
    artifacts = []
    page = 1
    while page <= 10:
        url = f"https://api.github.com/repos/{repo}/actions/artifacts?per_page=100&page={page}"
        data = gh_get_json(url, token)
        batch = data.get("artifacts", [])
        artifacts.extend(batch)
        if len(batch) < 100:
            break
        page += 1
    return artifacts


def pick_latest(artifacts, prefixes):
    candidates = []
    for a in artifacts:
        name = a.get("name", "")
        expired = bool(a.get("expired", False))
        if expired:
            continue
        if any(name.startswith(prefix) for prefix in prefixes):
            candidates.append(a)
    if not candidates:
        return None
    candidates.sort(key=lambda x: (x.get("updated_at") or x.get("created_at") or ""), reverse=True)
    return candidates[0]


def write_manifest(manifest_path: Path, rows: list[dict], repo: str, error: str = ""):
    manifest = {
        "generated_utc": datetime.now(timezone.utc).isoformat(),
        "repository": repo,
        "error": error,
        "profiles": rows,
    }
    manifest_path.parent.mkdir(parents=True, exist_ok=True)
    manifest_path.write_text(json.dumps(manifest, indent=2), encoding="utf-8")

    csv_path = manifest_path.with_suffix(".csv")
    headers = ["profile", "found", "artifact_id", "artifact_name", "created_at", "updated_at", "size_in_bytes", "status", "extracted_to"]
    lines = [",".join(headers)]
    for row in rows:
        def clean(v):
            s = str(v).replace('"', '""')
            return f'"{s}"'
        lines.append(",".join(clean(row.get(h, "")) for h in headers))
    csv_path.write_text("\n".join(lines), encoding="utf-8")


def main():
    p = argparse.ArgumentParser()
    p.add_argument("--output-dir", required=True)
    p.add_argument("--include-post-race-warning", action="store_true")
    args = p.parse_args()

    repo = os.environ.get("GITHUB_REPOSITORY", "").strip()
    token = os.environ.get("GITHUB_TOKEN", "").strip()

    if not repo:
        raise SystemExit("GITHUB_REPOSITORY is not set.")
    if not token:
        raise SystemExit("GITHUB_TOKEN is not set.")

    out = Path(args.output_dir)
    inputs = out / "inputs"
    downloads = out / "downloads"
    manifests = out / "manifests"
    for d in [inputs, downloads, manifests]:
        d.mkdir(parents=True, exist_ok=True)

    rows = []
    manifest_path = manifests / "elite_artifact_source_manifest.json"

    artifacts = list_artifacts(repo, token)

    for profile, prefixes in PROFILES.items():
        target = pick_latest(artifacts, prefixes)
        profile_dir = inputs / profile
        profile_dir.mkdir(parents=True, exist_ok=True)

        row = {
            "profile": profile,
            "prefixes": ";".join(prefixes),
            "found": False,
            "artifact_id": "",
            "artifact_name": "",
            "created_at": "",
            "updated_at": "",
            "size_in_bytes": "",
            "downloaded_to": "",
            "extracted_to": str(profile_dir),
            "status": "MISSING",
        }

        if target is None:
            rows.append(row)
            continue

        zip_path = downloads / f"{profile}_{target['id']}.zip"

        try:
            gh_download_artifact(target["archive_download_url"], token, zip_path)
            with zipfile.ZipFile(zip_path) as z:
                z.extractall(profile_dir)

            row.update({
                "found": True,
                "artifact_id": str(target.get("id", "")),
                "artifact_name": target.get("name", ""),
                "created_at": target.get("created_at", ""),
                "updated_at": target.get("updated_at", ""),
                "size_in_bytes": str(target.get("size_in_bytes", "")),
                "downloaded_to": str(zip_path),
                "status": "DOWNLOADED",
            })
        except Exception as e:
            row.update({
                "found": True,
                "artifact_id": str(target.get("id", "")),
                "artifact_name": target.get("name", ""),
                "created_at": target.get("created_at", ""),
                "updated_at": target.get("updated_at", ""),
                "size_in_bytes": str(target.get("size_in_bytes", "")),
                "downloaded_to": str(zip_path),
                "status": f"DOWNLOAD_FAILED: {type(e).__name__}: {e}",
            })
            rows.append(row)
            write_manifest(manifest_path, rows, repo, error=str(e))
            raise

        rows.append(row)

    write_manifest(manifest_path, rows, repo)

    found_count = sum(1 for r in rows if r["status"] == "DOWNLOADED")
    print(f"Downloaded {found_count}/{len(rows)} OpenF1 artifact profiles.")
    print(manifest_path)

    if found_count == 0:
        raise SystemExit("No OpenF1 artifacts downloaded; cannot run Elite Weekend Engine v2.")


if __name__ == "__main__":
    main()
